/**
 * BNB Chain Adapter
 * Uses @spp/bnb-validator-sdk for BNB Chain integration
 */
...
// Note: @spp/bnb-validator-sdk will need to be installed
// For now, we'll create the structure with proper typing
interface BNBValidatorSDK {
...
@Injectable()
export class BNBAdapter extends BaseChainAdapter {
  private sdk: BNBValidatorSDK;

  constructor() {
    super(BNBAdapter.name);
  }
  ...
  protected async initializeChain(): Promise<void> {
    try {
      ...
      this.logger.log('BNB Chain adapter initialized (SDK pending implementation)');

      // For now, create a mock SDK
      this.sdk = this.createMockSDK();
    } catch (error) {
      this.handleError('BNB Chain initialization', error);
    }
  }

  /**
   * Register a new match on BNB Chain
   */
  async registerMatch(params: MatchParams): Promise<TransactionResult> {
    this.ensureInitialized();

    try {
      this.logger.log(`Registering match ${params.matchId} on BNB Chain`);
  ...
  /**
   * Finalize a match on BNB Chain
   */
  async finalizeMatch(params: FinalizeMatchParams): Promise<TransactionResult> {
    this.ensureInitialized();

    try {
      this.logger.log(`Finalizing match ${params.matchId} on BNB Chain`);
  ...
  /**
   * Record general performance on BNB Chain
   */
  async recordPerformance(params: PerformanceParams): Promise<TransactionResult> {
    this.ensureInitialized();

    try {
      this.logger.log(`Recording performance for ${params.player} on BNB Chain`);
  ...
  async recordCricketPerformance(params: CricketPerformanceParams): Promise<TransactionResult> {
    ...
    try {
      this.logger.log(`Recording cricket performance for ${params.player} on BNB Chain`);
  ...
  /**
   * Execute burn transaction on BNB Chain
   */
  async executeBurn(params: BurnParams): Promise<BurnResult> {
    this.ensureInitialized();

    try {
      this.logger.log(`Executing burn for ${params.player} on BNB Chain`);
  ...
  /**
   * Get token balance on BNB Chain
   */
  ...
  /**
   * Check transaction status on BNB Chain
   */
  ...
  /**
   * Format address for BNB Chain (EVM compatible)
   */
  ...
  /**
   * Validate BNB Chain address (EVM format)
   */
   ...
   */
  private createMockSDK(): BNBValidatorSDK {
    return {
      initialize: async () => {},
